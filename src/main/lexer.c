...
/**
 * @ingroup Lexer internal
 * 
 * @brief Deallocates the twin buffer (automatically called at program termination).
 * 
 * @post `buffer` and all associated memory are freed and set to `NULL`.
 */
 ...
 * @ingroup Lexer internal
 * 
 * @brief Swaps the load and active buffer.
 * 
 * @param file_ptr A pointer to an opened file containing the source code.
 * 
 * @post `buffer.load_buffer` will contain the contents of the previous active buffer.
 *       `buffer.active_buffer` will be loaded with the next chunk from the file.
 */
 ...
 * @ingroup Lexer internal
 * 
 * @brief A helper function to retract the forward pointer and update buffer state.
 * 
 * @param no_of_times The number of times the forward pointer must be retracted.
 * 
 * @note If `no_of_times` is negative, the associated memory is freed (typically called at program termination).
 * 
 * @return A string representing the lexeme.
 * 
 * @warning The caller must **NOT** free the memory allocated for the token.
 *          Modifies the `buffer`. The forward pointer is reduced accordingly. 
 *          The begin pointer is set to the new forward pointer. 
 *          The active and load buffers are swapped if the new `forward_pointer` is negative and `is_loaded` is set to `true`.
 */
 ...
 * @ingroup Lexer internal
 * 
 * @brief An internal helper function that fetches the next token from the file.
 * 
 * @param file_ptr A pointer to an opened file containing the source code.
 * 
 * @details Uses a DFA (Deterministic Finite Automaton) to model the lexer.
 *          Updates static variables such as `state` and `line_number`.
 *          Must be called recursively until the state returns to zero or reaches a trap state.
 * 
 * @return NULL if the token hasn't been completed (i.e., in an intermediate state).
 *         A pointer to the next token if successful. 
 ...
 * @ingroup Lexer internal
 * 
 * @brief Helper function that allocates memory for a token and initializes it.
 * 
 * @param lexeme String representing the lexeme.
 * @param type The token type returned by the symbol table.
 * @param line The line number for error reporting.
 * 
 * @return NULL if memory allocation fails.
 *         A pointer to the token if successful. 
 * 
 * @warning The caller is responsible for freeing the memory allocated for the token.
 */
